...
1. クラスマスタから状態が "New" **かつ** 科目有効性が "TRUE" のクラスを抽出
2. システム設定から管理者アカウントIDを取得
3. 各クラスに対して:
   - Classroom APIでクラスを作成（オーナー: 管理者アカウント）
   - 作成されたクラスIDをクラスマスタのB列に記録
   - クラス状態を "Active" に更新
4. 処理結果を「登録処理ログ」に記録
5. エラーが発生した場合は「エラー未処理リスト」に記録

**重要**:
- Phase 2実行前に、システム設定シートで「管理者アカウントID」を設定してください
