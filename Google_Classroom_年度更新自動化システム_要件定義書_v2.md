...
| 8 | 一時作業スペース | マトリクス→縦持ち変換の作業領域 | 読み書き |
...
**サンプルデータ**:
```
| 科目名 | クラスID        | 履修登録マスタの列名 | 科目有効性 | クラス状態 | 担当教員名 | 最終オーナーメールアドレス |
|--------|-----------------|---------------------|-----------|-----------|-----------|------------------------|
| 国語   | GCA_JAP_001     | 国語                | TRUE      | Active    |           |                        |
| 数学   | GCA_MAT_002     | 数学                | TRUE      | New       |           |                        |
| 英語   | GCA_ENG_003     | 英語                | FALSE     | Active    |           |                        |
| 理科   | GCA_SCI_004     | 理科                | TRUE      | Active    |           |                        |
| 社会   | GCA_SOC_005     | 社会                | TRUE      | Active    |           |                        |
| 情報   | GCA_INF_006     | 情報                | TRUE      | New       |           |                        |
```
...
| C〜 | [科目名] | String | - | 履修している場合は「○」、していない場合は空欄 | `○` または空欄 |

**重要**: 
- C列以降の列名は「クラス（科目）マスタ」の「履修登録マスタの列名」と一致させること
- 横持ち（マトリクス）形式のため、GASで縦持ちに変換して処理する

**サンプルデータ**:
```
| 氏名     | 学籍番号  | 国語 | 数学 | 英語 | 理科 | 社会 |
|----------|----------|------|------|------|------|------|
| 田中 太郎 | S2025001 | ○    | ○    |      | ○    | ○    |
| 佐藤 花子 | S2025002 |      | ○    | ○    |      | ○    |
| 鈴木 健太 | S2025003 | ○    |      | ○    | ○    |      |
...
未実行 → 実行中 → 実行済（成功時）
                → エラー（失敗時）
未実行 → スキップ（アカウント無効等で処理対象外）
未実行 → 保留（手動確認が必要）
```

**サンプルデータ**:
```
| 学籍番号  | Googleメールアドレス   | 科目名 | Classroom ID | 登録要否 | 処理実行フラグ | 最終処理結果               | タイムスタンプ        |
|----------|----------------------|--------|--------------|---------|--------------|---------------------------|---------------------|
| S2025001 | taro.t@school.jp     | 国語   | GCA_JAP_001  | TRUE    | 実行済        | 成功                       | 2025-12-26 18:45:00 |
| S2025002 | hanako.s@school.jp   | 数学   | GCA_MAT_002  | TRUE    | エラー        | ERR_005: メールアドレス無効   | 2025-12-26 18:46:00 |
| S2025003 | kenta.s@school.jp    | 英語   | GCA_ENG_003  | TRUE    | 実行済        | 成功                       | 2025-12-26 18:46:00 |
| S2025004 | misaki.t@school.jp   | 理科   | GCA_SCI_004  | TRUE    | 保留          | アカウント有効性FALSE        | 2025-12-26 18:46:00 |
| S2025005 | daichi.k@school.jp   | 社会   | GCA_SOC_005  | FALSE   | スキップ      | 履修対象外                  | 2025-12-26 18:47:00 |
| S2025006 | yui.y@school.jp      | 情報   | GCA_INF_006  | TRUE    | 実行中        | 処理中                     | 2025-12-26 18:47:00 |
```
