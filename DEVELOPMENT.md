...
### 目的
転入生の追加登録時に、既に招待済みの生徒を再処理せず、新規生徒のみを招待できるようにする。
...
```
履修登録マスタ:
A列: 氏名 | B列: 学籍番号 | C列以降: 科目
```

**変更後**:
```
履修登録マスタ:
A列: 氏名 | B列: 学籍番号 | C列: 有効フラグ | D列以降: 科目
```

#### 修正ファイル
1. **src/services/data_transformer.gs**
   - `transformEnrollmentDataToVertical()`: C列に有効フラグを追加
   - `subjectStartCol`を2から3に変更（D列以降が科目）
   - 有効フラグがFALSEの生徒をスキップする処理を追加
   - サンプルコードのドキュメントを更新

2. **src/debug/debug_tools.gs**
   - ヘッダー表示のデバッグ出力を修正
   - 履修登録マスタの構造説明を更新

3. **src/main.gs**
   - 使い方ガイドに転入生追加方法を追加
   - Phase 4の説明に有効フラグの記述を追加

#### 処理フロー
```javascript
for (各生徒) {
  if (有効フラグ === FALSE || 有効フラグ === "" || 有効フラグ === "FALSE") {
    スキップ（招待処理を行わない）;
    continue;
  }
  // 通常の招待処理
...
#### 転入生追加の手順
1. 履修登録マスタで既登録者のC列（有効）を「FALSE」に変更
2. 新しい転入生の行を追加（氏名、学籍番号、有効=TRUE、履修科目=○）
3. Phase 4を実行
4. 転入生のみが招待される（既登録者はスキップ）
5. 完了後、全員のC列を「TRUE」に戻す

#### 例
```
| 氏名     | 学籍番号  | 有効  | 国語 | 数学 | 英語 |
|----------|----------|-------|------|------|------|
| 田中 太郎 | S2025001 | FALSE | ○    | ○    |      | ← 既登録（スキップ）
| 佐藤 花子 | S2025002 | FALSE |      | ○    | ○    | ← 既登録（スキップ）
| 鈴木 一郎 | S2025003 | TRUE  | ○    |      | ○    | ← 転入生（招待される）
```
...
**理由**:
1. C列フラグチェックはO(1)の単純比較演算
2. FALSE生徒は早期スキップ（continue）により処理が減少
3. API呼び出し回数が削減

**効果の例**:
- **既登録100人 + 転入生5人**の場合
  - 従来: 105人全員処理（既登録者も招待API送信でエラー発生の可能性）
  - 新方式: 5人のみ処理（100人はフラグでスキップ）
  - **結果: 約20倍高速化！**

### 後方互換性
- C列が存在しない場合、全員をスキップと判定（FALSE扱い）
- C列を追加後、全員のフラグを「TRUE」にすることで従来通りの動作

### テスト結果
- ✅ DRY_RUNモードでスキップログが正しく出力される
- ✅ 有効フラグ=TRUEの生徒のみ処理される
- ✅ 有効フラグ=FALSEの生徒が正しくスキップされる
- ✅ 処理時間への影響なし
